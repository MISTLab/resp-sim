...
// KIND macros are the same as SIZE, but if SIZE greater than 8 or ALIGN greater than SIZE saturate to 8
...
{
  char test[(LOCKSIZE != sizeof (omp_lock_t)
         || 4 != __alignof (omp_lock_t)
         || NLOCKSIZE != sizeof (omp_nest_lock_t)
         || 4 != __alignof (omp_nest_lock_t)
         || LOCKSIZE != sizeof (*(omp_lock_arg_t) 0)
         || NLOCKSIZE != sizeof (*(omp_nest_lock_arg_t) 0))
