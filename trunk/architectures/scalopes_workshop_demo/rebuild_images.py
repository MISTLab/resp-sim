...
def write_diff_image(filename, pixels, check):
...
#      pixels[3*i+2] = tmp
    for i in range(0,pixels.__len__()/3/80):
      for j in range(0,80):
        if check[i*80*3 + (79-j)*3] == pixels[i*80*3 + (79-j)*3] and check[i*80*3 + (79-j)*3+1] == pixels[i*80*3 + (79-j)*3+1] \
        and check[i*80*3 + (79-j)*3+2] == pixels[i*80*3 + (79-j)*3+2]:        
    ...
    for i in range(0,pixels.__len__()/80):
      for j in range(0,80):
        if check[i*80 + (79-j)] == pixels[i*80 + (79-j)]:        
...
def write_image(filename, pixels):
  f_out = open(filename,"wb")
  w=WIDTH
  w_1=w & 255
  w_2=(w >> 8) & 255
