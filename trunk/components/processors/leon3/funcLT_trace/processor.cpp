...
void leon3_trace_trap::Processor::mainLoop(){
    ...
    while(true){
        ...
        std::cerr << "Current PC: " << std::hex << std::showbase << this->PC-4 << std::endl;
        ...
        if(cachedInstr != instrCacheEnd){
            ...
            try{
                ...
                if(!(this->toolManager.newIssue(this->PC-4, cachedInstr->second))){
                    ...
                    std::cerr << "Not executed Instruction because Tools anulled it" << std::endl << \
                        std::endl;
                }
                #endif
            }
                ...
                cachedInstr->second->printTrace();
                std::cerr << "Skipped Instruction " << cachedInstr->second->getInstructionName() \
                    << std::endl << std::endl;
            ...
            try{
                ...
                instr->printTrace();
                std::cerr << "Skipped Instruction " << instr->getInstructionName() << std::endl << \
                    std::endl;
...
void leon3_trace_trap::Processor::resetOp(){
...
Instruction * leon3_trace_trap::Processor::decode( unsigned int bitString ){
    int instrId = this->decoder.decode(bitString);
    if(instrId >= 0){
        Instruction * instr = Processor::INSTRUCTIONS[instrId];
        instr->setParams(bitString);
...
void leon3_trace_trap::Processor::end_of_elaboration(){
    this->resetOp();
}

Instruction * * leon3_trace_trap::Processor::INSTRUCTIONS = NULL;
template_map< unsigned int, Instruction * > leon3_trace_trap::Processor::instrCache;
int leon3_trace_trap::Processor::numInstances = 0;
leon3_trace_trap::Processor::Processor( sc_module_name name, sc_time latency ) : \
...
leon3_trace_trap::Processor::~Processor(){
    Processor::numInstances--;
