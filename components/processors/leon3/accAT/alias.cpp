...
void leon3_accat_trap::Alias::setPipeId( unsigned int pipeId ){
    if(this->pipeId < 0){
        this->pipeId = pipeId;
    }
    else{
...
void leon3_accat_trap::Alias::writeAll( const unsigned int & value ) throw(){
    this->pipelineReg->writeAll(value);
}

unsigned int leon3_accat_trap::Alias::operator ~() throw(){
    return ~*this->pipelineReg->getRegister(this->pipeId);
}

Alias & leon3_accat_trap::Alias::operator =( const Alias & other ) throw(){
...
bool leon3_accat_trap::Alias::operator <( const Register & other ) const throw(){
...
Alias & leon3_accat_trap::Alias::operator =( const Register & other ) throw(){
...
std::ostream & leon3_accat_trap::Alias::operator <<( std::ostream & stream ) const \
    throw(){
    stream << *this->pipelineReg->getRegister(this->pipeId);
    return stream;
}

void leon3_accat_trap::Alias::updateAlias( Alias & newAlias ) throw(){
...
void leon3_accat_trap::Alias::updateAlias( PipelineRegister & newAlias ) throw(){
...
void leon3_accat_trap::Alias::directSetAlias( Alias & newAlias ) throw(){
    this->pipelineReg = newAlias.pipelineReg;
...
void leon3_accat_trap::Alias::directSetAlias( PipelineRegister & newAlias ) throw(){
    this->pipelineReg = &newAlias;
    if(this->referringAliases != NULL){
        this->referringAliases->referredAliases.remove(this);
    }
...
leon3_accat_trap::Alias::Alias( PipelineRegister * pipelineReg, int pipeId ) : pipelineReg(pipelineReg), \
    pipeId(pipeId){
    this->referringAliases = NULL;
}

leon3_accat_trap::Alias::Alias( int pipeId ) : pipelineReg(NULL), pipeId(pipeId){
    this->referringAliases = NULL;
}

leon3_accat_trap::Alias::Alias() : pipelineReg(NULL), pipeId(-1){
    this->referringAliases = NULL;
}

leon3_accat_trap::Alias::Alias( Alias * initAlias, int pipeId ) : pipelineReg(initAlias->pipelineReg), \
    pipeId(pipeId){
    initAlias->referredAliases.push_back(this);
    this->referringAliases = initAlias;
}

leon3_accat_trap::Alias::~Alias(){
    std::list<Alias *>::iterator referredIter, referredEnd;
    for(referredIter = this->referredAliases.begin(), referredEnd = this->referredAliases.end(); \
        referredIter != referredEnd; referredIter++){
