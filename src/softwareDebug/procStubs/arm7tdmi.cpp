...
arm7Stub::arm7Stub(arm7 & processor,  unsigned int verbosityLevel) : processor(processor),  verbosityLevel(verbosityLevel){}
 ...
 */
bool arm7Stub::get_ac_tgt_endian(){
 ...
 */  
unsigned int arm7Stub::nRegs(){
   #ifndef NDEBUG
   if(verbosityLevel >= 2)
      std::cerr << __PRETTY_FUNCTION__ << ": determining number of registers --> 26" << std::endl;
   #endif
   return 26;
}
 ...
 */
unsigned int arm7Stub::reg_read(int reg, MySQL *mySQL, double curSimTime){
    #ifdef ENABLE_MYSQL
    if(curSimTime < sc_time_stamp().to_double()){
        ...
        #ifndef NDEBUG
        if(verbosityLevel >= 2)
            std::cerr << __PRETTY_FUNCTION__ << ": reading register " << reg << " at time " << curSimTime << std::endl;
        #endif
        try{
            SingleEvent ev;
            if(reg < 15)
            ...
            #ifndef NDEBUG
            std::cerr << __PRETTY_FUNCTION__ << ": Read Register " << ev.value << std::endl;
            #endif
            return ev.value;
        }
        catch(std::exception &e){
            #ifndef NDEBUG
            std::cerr << __PRETTY_FUNCTION__ << ": Exception occurred during the reading of register " << reg << " from the database" <<std::endl;
            std::cerr << __PRETTY_FUNCTION__ << " ERROR MESSAGE --> " << e.what() <<std::endl;            
            #endif
           return 0;
        }
        catch(...){
            #ifndef NDEBUG
            std::cerr << __PRETTY_FUNCTION__ << ": Generic Exception occurred during the reading of register " << reg << " from the database" <<std::endl;
            #endif
           return 0;
        }
    }
    ...
    #ifndef NDEBUG
    if(verbosityLevel >= 2)
        std::cerr << __PRETTY_FUNCTION__ << ": reading register " << reg << std::endl;
    #endif

    if(reg < 15){
        #ifndef NDEBUG
        if(verbosityLevel >= 2)
            std::cerr << __PRETTY_FUNCTION__ << ": Value " << std::hex << this->processor.RB.read(reg) << std::dec << std::endl;
        #endif
        ...
        #ifndef NDEBUG
        if(verbosityLevel >= 2)
            std::cerr << __PRETTY_FUNCTION__ << ": Value " << std::hex << (unsigned int)this->processor.ac_pc << std::dec << std::endl;
        #endif
        return (unsigned int)this->processor.ac_pc;
    }
    else if(reg == 25){
        #ifndef NDEBUG
        if(verbosityLevel >= 2)
            std::cerr << __PRETTY_FUNCTION__ << ": Value " << std::hex << this->processor.PSR.read(0) << std::dec << std::endl;
        #endif
...
*/
void arm7Stub::reg_write(int reg, unsigned int value, MySQL *mySQL, double curSimTime){
    #ifdef ENABLE_MYSQL
    if(curSimTime < sc_time_stamp().to_double()){
 ...
 */
void arm7Stub::set_ac_pc(unsigned int value, MySQL *mySQL, double curSimTime){
    #ifdef ENABLE_MYSQL
    if(curSimTime < sc_time_stamp().to_double()){
        THROW_EXCEPTION("Modifying the past simulation status not yet allowed");
    }
 ...
 */
unsigned char arm7Stub::mem_read(unsigned int address, MySQL *mySQL, double curSimTime){
    #ifdef ENABLE_MYSQL
    if(curSimTime < sc_time_stamp().to_double()){
        //Ok,  I'm trying to perform debug in the past, so I need to access the database;
        //I have to read the last value of the register before curSimTime (included)
 ...
 */
void arm7Stub::mem_write(unsigned int address, unsigned char byte, MySQL *mySQL, double curSimTime){
    #ifdef ENABLE_MYSQL
    if(curSimTime < sc_time_stamp().to_double()){
        THROW_EXCEPTION("Modifying the past simulation status not yet allowed");
    }
...
/**
  ...
  */
double arm7Stub::findEvent(Breakpoint<unsigned int> &breakpoint, double startTime, double endTime, MySQL *mySQL){
    ...
    try{
        ...
        #ifndef NDEBUG
        std::cerr << __PRETTY_FUNCTION__ << ": Exception occurred during the check of the breakpoints from the database" <<std::endl;
        #endif
        return -1;
    }
    return -1;
...
/**
  ...
  */
double arm7Stub::getNextInstruction(bool forward, double simTime, MySQL *mySQL){
