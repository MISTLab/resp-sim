...
///This parameter uses the average load as statistic. Given
///this the only action that can be performed is an increment/decrement
///of the processor frequency according to the possible frequencies specified at configuration
///time in the configurtion file.
class ProcFreqIf : public IntegerPlugin{
  public:
    ProcFreqIf(const std::vector<std::string> &values, std::string pluginName);
    ...
    unsigned int getProcFrequency(int frequencyEnum);
};

class ProcFreqCreator : public PluginCreator{
  public:
    ProcFreqCreator();
    ~ProcFreqCreator();
