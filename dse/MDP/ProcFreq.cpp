...
///one
std::pair<float, float> ProcFreqIf::changeValue(plugin_int_map &parameters, int newValue, const std::string &metric,
...
void ProcFreqIf::updateStatistics(float_map &curStats, int oldValue, int action, const std::map<PluginIf *, std::string> &parameter_values){
}
...
///this value
void ProcFreqIf::getStats(RespClient &client, float_map &toUpdateStats){
