...
class CacheSubstsPolicyIf : public StringPlugin{
  protected:
    ECacti model;
  public:
    CacheSubstsPolicyIf(const std::vector<std::string> &values, std::string pluginName);
    ...
    std::string getCachePolicy(int sizeEnum);
};

class CacheSubstsPolicyCreator : public PluginCreator{
  public:
    CacheSubstsPolicyCreator();
    ~CacheSubstsPolicyCreator();
