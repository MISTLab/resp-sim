...
///one
std::pair<float, float> MemLatencyIf::changeValue(plugin_int_map &parameters, int newValue, const std::string &metric,
...
void MemLatencyIf::updateStatistics(float_map &curStats, int oldValue, int action, const std::map<PluginIf *, std::string> &parameter_values){
}
...
///this value
void MemLatencyIf::getStats(RespClient &client, float_map &toUpdateStats){
